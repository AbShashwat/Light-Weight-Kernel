#include <stdint.h>  // Corrected to use standard C header for fixed-width integer types
...
ATA::ATA(uqaabOS::interrupts::InterruptManager* interrupt_manager, bool master, uint16_t port_base)
    : InterruptHandler(interrupt_manager, 0x20 + 14), // IRQ 14 for primary ATA
      ...
      sector_count_port(port_base + 0x2), // Initialize sector count port at base + 2.
      ...
      device_port(port_base + 0x6), // Initialize device register port at base + 6.
      command_port(port_base + 0x7), // Initialize command port at base + 7.
      control_port(port_base + 0x206) // Initialize control port at base + 0x206.
{
...
void ATA::identify() {
  ...
  device_port.write(master ? 0xA0 : 0xB0); // Re-select device based on master flag.
...
void ATA::write28(uint32_t sector_num, uint8_t *data, uint32_t count) {
  if (sector_num > 0x0FFFFFFF) // Validate that the sector number fits in 28 bits.
    return;
  if (count > 512) // Ensure that no more than one sector (512 bytes) is written.
    return;
  ...
  lba_high_port.write((sector_num & 0x00FF0000) >> 16); // Write the LBA high byte.
  ...
  for (int i = 0; i < count; i += 2) {
    ...
    if (i + 1 < count)
      wdata |= ((uint16_t)data[i + 1]) << 8;        // Combine with the second byte if available.
...
void ATA::flush() {
  device_port.write(master ? 0xE0 : 0xF0); // Select the device (master or slave).
  command_port.write(0xE7);         // Send the FLUSH command (0xE7).
  uint8_t status = command_port.read(); // Read the status after the flush command.
