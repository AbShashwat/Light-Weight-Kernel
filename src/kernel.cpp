...
#include <stdint.h> // Corrected from <stdint>
...
void taskA() {
  for (int i = 0; i < 100; i++) {
    ...
    if (i % 10 == 9) {
      asm volatile("int $0x20");  // Trigger context switch (int $0x20 for IRQ0)
    }
  }
}
...
extern "C" void kernel_main(const void *multiboot_structure,
  ...
  uint32_t *memupper =
      (uint32_t *)(((uqaabOS::memorymanagement::size_t)multiboot_structure) +
                   8);
  uint32_t heap = 10 * 1024 * 1024;  // Adjusted to uint32_t
  ...
  uqaabOS::libc::printf("\nallocated: ");
  uqaabOS::libc::print_hex((uintptr_t)allocated);  // Changed to uintptr_t
  uqaabOS::libc::printf("\n");
